...
def _fisher_exact(list_a: List[float], list_b: List[float]) -> Tuple[float, float]:
...
def fisher_exact(*lists, adjust: bool = True) -> List[Tuple[float, float]]:
    if len(lists) == 2:
        odds_ratio, p_value = _fisher_exact(list_a=lists[0], list_b=lists[1])
        return [(round(odds_ratio, 2), round(p_value, 4))]
    p_values = []
    odds_ratios = []
    for i in range(len(lists)):
        for j in range(i + 1, len(lists)):
            odds_ratio, p_value = _fisher_exact(list_a=lists[i], list_b=lists[j])
